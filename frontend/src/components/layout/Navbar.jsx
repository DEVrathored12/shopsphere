            ...
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  ...
                  isActive ? "text-accent" : "text-primary hover:bg-background"
                )
              }
            >
              {link.label}
        ...
        <div className="hidden lg:flex items-center gap-3">
          {isAuthenticated ? (
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white border border-border rounded-xl shadow-lg py-1.5 overflow-hidden">
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-primary hover:bg-background"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-danger hover:bg-background"
                  >
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-border bg-white">
          <div className="container-app py-3 flex flex-col gap-1">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive ? "text-accent bg-accent/5" : "text-primary hover:bg-background"
                  )
                }
              >
                {link.label}
            ...
            {isAuthenticated ? (
              <>
                <Link to={dashboardPathFor(user?.role)} onClick={() => setMobileOpen(false)} className="px-3 py-2.5 rounded-lg text-sm font-medium text-primary hover:bg-background">
                  Dashboard
                </Link>
                {user?.role === "customer" && (
                  <Link to="/favorites" onClick={() => setMobileOpen(false)} className="px-3 py-2.5 rounded-lg text-sm font-medium text-primary hover:bg-background">
                    Favorites
                  </Link>
                )}
                {user?.role === "customer" && (
                  <Link to="/recently-viewed" onClick={() => setMobileOpen(false)} className="px-3 py-2.5 rounded-lg text-sm font-medium text-primary hover:bg-background">
                    Recently Viewed
                  </Link>
                )}
                {profilePath && (
                  <Link to={profilePath} onClick={() => setMobileOpen(false)} className="px-3 py-2.5 rounded-lg text-sm font-medium text-primary hover:bg-background">
                    Profile
                  </Link>
                )}
                <button type="button" onClick={handleLogout} className="text-left px-3 py-2.5 rounded-lg text-sm font-medium text-danger hover:bg-background">
                  Logout
                </button>
              </>
            ) : (
              <div className="flex gap-2 px-3 pt-1">
                ...
                <Button className="flex-1" onClick={() => { setMobileOpen(false); navigate("/register"); }}>
                  Register
                </Button>
              </div>
            )}
