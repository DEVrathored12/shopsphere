...
import { SlidersHorizontal, Store, Package } from "lucide-react";
...
export default function Explore() {
  ...
  const tab = searchParams.get("tab") === "products" ? "products" : "shops";
  ...
  // Keep ?q in sync with the debounced search input (no request per keystroke).
  useEffect(() => {
  ...
  const switchTab = (nextTab) => updateParams({ tab: nextTab === "shops" ? "" : nextTab, sort: "" });
  ...
  const queryParams = useMemo(() => {
    ...
    return params;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, debouncedSearch, filters, sort, coords, tab]);

  const results = useAsync(
    () => (tab === "shops" ? fetchShops(queryParams) : fetchProducts(queryParams)),
  ...
  return (
    <div className="container-app py-8 sm:py-10">
      ...
      {/* ================= FILTERS MODAL (also serves as the mobile sheet) ================= */}
